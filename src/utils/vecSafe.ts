...
export function vecSafe(vec: Cesium.Cartesian3): Cesium.Cartesian3 {
  ...
  if (magnitude < 1e-6 || !isFinite(magnitude)) {
...
/**
 * Returns true if the vector is valid (not NaN or near-zero magnitude).
 */
...
export function safeClone(vec: Cesium.Cartesian3): Cesium.Cartesian3 {
  return isValidVec(vec) ? Cesium.Cartesian3.clone(vec, new Cesium.Cartesian3()) : new Cesium.Cartesian3(0, 0, 1);
}
